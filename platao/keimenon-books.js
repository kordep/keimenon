...
(()=>{
  ...
  const $=id=>document.getElementById(id),t=key=>app?.labels?.[app.lang()]?.[key]||(L[app?.lang()]||L.pt)[key]||key;
  const key=(s,r)=>s+':'+r;
  ...
  async function open(s='warner',second=false,file=null){
    ...
    const ids=app.editions?.map(e=>e.id)||['warner','canfora'];
    source=ids.includes(s)?s:ids[0];asSecond=second;$('bookEdition').value=source;
  ...
  function mount(adapter){
    app=adapter;source=app.editions?.[0]?.id||'warner';
    ...
    if(app.editions)$('bookEdition').replaceChildren(...app.editions.map(e=>new Option(e.label,e.id)));
    if(app.refPlaceholder){$('bookRef').placeholder=app.refPlaceholder;$('bookRef').inputMode='text';}
    translate();
    ...
    $('bookSave').onclick=async()=>{
      const m=$('bookRef').value.trim().match(/^([1-8])[.,](\d{1,3})$/),ref=app.normalizeRef?app.normalizeRef($('bookRef').value):(m?m[1]+'.'+Number(m[2]):''),text=$('bookText').value.trim();
    ...
    api.ready=(async()=>{
      try{db=await new Promise((resolve,reject)=>{const req=indexedDB.open(app.database||'keimenon_books_v1',1);req.onupgradeneeded=()=>{for(const s of ['files','meta','chapters'])req.result.createObjectStore(s);};req.onsuccess=()=>resolve(req.result);req.onerror=()=>reject(req.error);req.onblocked=()=>reject(new Error('blocked'));});
      ...
      if(app.pageHints!==false)try{const r=await fetch(app.pageHints||'book-page-hints.json');if(r.ok)hints=await r.json();}catch(e){}
      translate();
    })();
  }
